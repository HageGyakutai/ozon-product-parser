...
def test_nested_serialized_json_and_non_json_text():
    state = {"widget": json.dumps({"items": [json.dumps({
        "@type": "Product", "sku": "123", "name": "Nested",
...
def test_selects_matching_sku_without_mixing_other_product_fields():
    other = {"@type": "Product", "sku": "456", "name": "Other", "price": 1,
...
def test_price_rating_reviews_cover_media_and_characteristics():
    state = {"@type": "Product", "sku": "123", "name": "Item", "price": "999",
...
@pytest.mark.parametrize(("field", "value"), [
    ("sku", "123"), ("title", "Item"), ("price", Decimal("15.50")),
    ("rating", Decimal("4.9")), ("reviews_total", 9),
    ("cover_image", "https://example.org/cover.jpg"),
    ("photos_seller", 3), ("videos_seller", 2), ("color", "Белый"),
    ("material", "Хлопок"), ("art_set", "A-1"), ("has_rich_content", True),
])
def test_each_required_field(field, value):
    state = {"@type": "Product", "sku": "123", "name": "Item",
             "offers": {"price": "15,50"},
             "aggregateRating": {"ratingValue": "4.9", "reviewCount": 9},
             "image": "https://example.org/cover.jpg",
             "photosSeller": 3, "videosSeller": 2,
             "characteristics": [
...
def test_price_uses_product_fallback_if_offers_have_no_valid_price():
    state = {"@type": "Product", "sku": "123", "name": "Item",
             "price": "90", "offers": [{"price": None}, {"price": "unknown"}]}
