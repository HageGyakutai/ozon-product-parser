...
def wait_for_code(service, since: datetime, timeout: int = 180, interval: int = 5) -> str:
    cutoff = int(since.astimezone(timezone.utc).timestamp() * 1000)
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        result = service.users().messages().list(userId="me", q=f"after:{cutoff // 1000} ozon", maxResults=20).execute()
        for item in result.get("messages", []):
            ...
            def collect(part):
                ...
                if encoded and part.get("mimeType") in ("text/plain", "text/html"):
                    chunks.append(base64.urlsafe_b64decode(encoded + "===").decode("utf-8", errors="replace"))
