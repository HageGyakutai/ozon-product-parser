...
В открытом браузере самостоятельно пройдите вход по номеру телефона и подтвердите вход. Затем вернитесь в терминал и введите `SAVE` (пустой Enter отменяет сохранение). Скрипт сохранит cookies в `cookies.json` с доступом только владельцу. Успех входа определяется пользователем; наличие cookies домена Ozon само по себе не подтверждает авторизацию. Если Ozon показывает «Похоже, нет соединения» или окно Chromium не появилось в WSL, не вводите `SAVE`: сначала проверьте графический интерфейс WSL (`echo "$DISPLAY"` и `echo "$WAYLAND_DISPLAY"`).
