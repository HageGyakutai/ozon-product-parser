...
**Статус:** OAuth и запрос к Gmail API проверены на реальном аккаунте. Реальный HTML одной карточки Ozon уже сохранён вручную из обычного Chrome, успешно разобран `extractor.py` и записан в PostgreSQL с `EXIT_CODE=0`. Получение кода Ozon через Gmail, автоматический вход на живой сайт, live-загрузка карточки автоматизированным клиентом и корректность всех 12 полей ещё не подтверждены. Сейчас Ozon отправляет код на почту другого провайдера, а ТЗ требует именно Gmail API. Последний GitHub Actions заблокирован до запуска шагов: GitHub сообщает о неудавшемся платеже по аккаунту или необходимости увеличить лимит расходов. Исправление находится в Billing & plans владельца аккаунта; зелёный CI для текущего кода ещё не подтверждён. В WSL Playwright Chromium и Firefox показывают `Antibot Challenge Page`. Число фото/видео продавца нужно сверить с настоящим JSON.
