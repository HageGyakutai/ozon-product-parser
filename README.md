...
**Статус:** OAuth и запрос к Gmail API проверены на реальном аккаунте. Получение кода Ozon через Gmail, вход на живой сайт, структура карточки и полный сценарий ещё не подтверждены. Сейчас Ozon отправляет код на почту другого провайдера, а ТЗ требует именно Gmail API. Последние GitHub Actions падают: в ранних запусках на форматировании, в последних job завершается до запуска шагов; результат после новых изменений ещё не известен. В WSL Playwright Chromium и Firefox показывают `Antibot Challenge Page`. Число фото/видео продавца нужно сверить с настоящим JSON.
...
Workflow CI настроен на сборку образа, миграцию и тесты на отдельной PostgreSQL-базе. **Последние запуски CI не прошли**, поэтому проверяйте текущий результат на странице Actions. Локально для тестов БД поднимите отдельную пустую базу:
