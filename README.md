...
**Статус:** автоматически проверены парсинг синтетического embedded JSON, перенос session metadata, локальная логика Gmail и CLI; ранее в GitHub Actions проверялись PostgreSQL/Alembic и интеграционные тесты. Результат CI для самых последних изменений отдельно не подтверждён. Реальный OAuth Gmail и запрос `users.getProfile` успешно проверены пользователем. Письмо с кодом Ozon, вход в Ozon, структура живой карточки и полный сценарий остаются открытыми. Сейчас код направляется на почту другого провайдера, поэтому требуемое ТЗ получение кода именно через Gmail API невозможно подтвердить до изменения адреса получения кода или согласования изменения ТЗ. В WSL Playwright Chromium и Firefox получают `Antibot Challenge Page` до формы входа. Число картинок в `image` не равно гарантированно числу фото продавца; `photos_seller` и `videos_seller` нужно сверить с настоящим JSON.
...
|---|---|
| Два скрипта | `get_cookies.py` отвечает за вход и сохранение cookies; `parse_ozon.py` читает их и обрабатывает список SKU. |
| Номер телефона и Gmail API | Номер из `.env`; OAuth и запрос Gmail API проверены на реальном аккаунте. Получение письма Ozon пока не подтверждено: Ozon отправляет его на другую почту. |
| Карточки `www.ozon.ru/product/{sku}/` | Запросы с сохранённой сессией реализованы; доступ и формат живого HTML ещё не проверены. `data.ozon.ru` используется только для входа. |
| 12 полей товара | Извлечение и типы проверены на синтетическом HTML; реальные пути до данных ещё не подтверждены. Отсутствующие необязательные характеристики допускают `NULL`. |
| Результат | PostgreSQL реализован, CSV доступен дополнительно. Локально 79 тестов проходят, 10 PostgreSQL-тестов пропущены без `TEST_DATABASE_URL`. |
| Логи и исключения | Вход, ожидание письма и результаты по SKU логируются без телефона, кода, cookies и содержимого письма. Предусмотрены ошибки отсутствия письма, страницы и доступа. |
| Airflow / DataLens | Необязательные бонусы, после проверки основного сценария. |
